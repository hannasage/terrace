...
- [ ] **[H]** Read the current Claude Code hooks reference and confirm the
...
- [ ] **[CC]** "Implement `pipeline/ingest/` for engsoccerdata and
...
- [ ] Confirm `ci.yml` passes end to end on a pull request.
...
- [ ] **[H]** Open a second test pull request touching only a file under
      `pipeline/data/` and confirm no review is requested.

## Phase 6: the automation token

- [ ] **[H]** Create a fine-grained personal access token scoped to this
      repository only, with Contents write and Pull requests write. Nothing
      else.
- [ ] **[H]** Store it as the repository secret `AUTOMATION_TOKEN`.
- [ ] **[H]** Set a calendar reminder for its expiry. A silently expired token
      looks exactly like a data source that stopped changing.
- [ ] **[H]** Trigger `data-refresh` manually from the GitHub app or the
      Actions tab and watch one full cycle: pull request opens, checks run
      without anyone approving them, merge happens unattended.

That last box is the one that proves the whole design. If the checks sit in an
approval-required state, the token is not being used for `gh pr create`.

## Phase 7: deployment

- [ ] **[H]** Connect the repository to Vercel. Production branch `main`.
- [ ] **[H]** Add the subdomain on hannasage.love.
- [ ] **[H]** Configure deployment protection. An unlisted subdomain is not a
      private one, and this is currently unconfigured.
- [ ] **[CC]** "Wire the published Parquet artefacts into `web/public/data/`
      and confirm DuckDB-WASM queries them over range requests in a production
      build."

## Phase 8: the remote surfaces

Once the mini is running, or on the MacBook in the meantime.

- [ ] **[H]** Host energy settings: prevent sleep, restart after power failure,
      wake for network. Automatic login on. Claude Desktop launching at login.
- [ ] **[H]** Pair Dispatch: Claude Desktop, Cowork tab, Dispatch setup, scan
      the QR code from the phone.
- [ ] **[H]** Assign one throwaway task through Dispatch and confirm the
      session appears in the Desktop Code tab with a Dispatch badge and that a
      push notification arrives.
- [ ] **[H]** On the host, `/config`: enable Remote Control for all sessions,
      push when Claude decides, and push when actions required.
- [ ] **[H]** Confirm a Remote Control session is reachable from the phone.
- [ ] **[H]** GitHub mobile app: notifications on for failed workflow runs and
      for review requests.

## Phase 9: measurement

- [ ] **[H]** Run `automation-metrics` manually once and confirm it writes
      `docs/metrics/automation.json` and commits.
- [ ] **[CC]** "Record the M0 baseline in `docs/DECISIONS.md` per D-008."
